...
WHAT IT RUNS, in order:
  check_fidelity   schema pass, one item one subject, one story one lane, the belief and its
                   order, the word ceiling, LinkedIn laws. Cadence distributions OFF unless
                   --cadence (3.11.0).
  hook_lint        first line and last line, the epigram, banned words
  spoken_lint      verbless runs, rejected phrases, absence claims; cadence floors with --cadence
  pack             show/<date>-filming-pack.md is newer than the week file and names only
                   episodes the file carries (build_pack.py renders it)
...
Usage:
  ...
  python3 radar_gate.py --week ... --cadence               # grade the cadence distributions
  python3 radar_gate.py --week ... --strict-cadence        # and make them FAIL
...
def completeness(d):
    ...
    if outliers == 0:
        lines.append("warn: inspiration[] is empty, so no outlier was banked this week. The sweep is "
                     "where borrowed mechanics come from; say in sweep_note if the week has none.")
        rc = max(rc, 1)
    else:
...
def pack_check(wpath, d):
    """The filming pack is what the creator reads to camera. It used to be typed by hand and
    regenerated by nobody, so on 2026-09-21 it still named two episodes the week file had
    replaced. (rc, lines). Missing: warn. Older than the week file, or naming an episode no
    item carries: FAIL. `build_pack.py --week <file>` renders it."""
    lines = []
    home = str(HOME)
    week = (d.get("week") or "").split()[0] if d.get("week") else os.path.basename(wpath)[:-5]
    rel = os.path.relpath(wpath, home) if wpath.startswith(home) else wpath
    pack = os.path.join(home, "show", f"{week}-filming-pack.md")
    if not os.path.exists(pack):
        return 1, [f"warn: no show/{week}-filming-pack.md. Render it: python3 build_pack.py --week {rel}"]
    if os.path.getmtime(pack) < os.path.getmtime(wpath):
        lines.append(f"FAIL: the pack is older than the week file. Re-render: python3 build_pack.py --week {rel}")
    text = open(pack, encoding="utf-8").read()
    known = " || ".join(f"{it.get('company') or ''} {it.get('title') or ''}"
                        for it in (d.get("distribution") or []) if isinstance(it, dict)).lower()
    for _n, subj in PACK_HEAD.findall(text):
        sj = subj.strip()
        if sj and sj.lower() not in known:
            lines.append(f"FAIL: the pack names an episode about {sj!r} and no item carries it. "
                         f"Re-render: python3 build_pack.py --week {rel}")
    rc = 2 if any(l.startswith("FAIL") for l in lines) else (1 if lines else 0)
    if not lines:
        lines.append(f"pack: show/{week}-filming-pack.md is current")
    return rc, lines


def gate_week(wpath, skip, allow_unvalidated, strict_cadence, verbose, cadence=False):
    ...
    if "pack" in skip:
        skipped.append("pack")
        rows.append(("pack", "-", "skipped"))
    else:
        rc, lines = pack_check(wpath, d)
        n_f = sum(1 for l in lines if l.startswith("FAIL"))
        n_w = sum(1 for l in lines if l.startswith("warn"))
        record("pack", rc, lines[0][:110] if not (n_f or n_w) else
               f"{n_f} fail, {n_w} warn: " + lines[0][:80])
        if verbose or n_f or n_w:
            for l in lines:
                print(f"  pack: {l}")
...
def main():
    ...
    ap.add_argument("--cadence", action="store_true",
                    help="grade the cadence distributions (off by default since 3.11.0)")
