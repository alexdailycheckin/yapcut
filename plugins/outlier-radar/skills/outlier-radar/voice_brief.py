...
run, has to remember to ground itself. That is what this file is.
    ...
    python3 scripts/voice_brief.py                  # the brief, for pasting into a writer prompt
    python3 scripts/voice_brief.py --week 2026-09-07  # deterministic sample rotation per week
    python3 scripts/voice_brief.py --samples 24     # more few-shot passages

Regenerate targets first if the corpus has grown:  python3 scripts/derive_voice_targets.py
"""
import json
import os
import pathlib
...
def main():
    ...
    # off-topic small talk. Showing a model the wrong register and demanding the right one
    # is the same class of error as describing the voice instead of showing it.
    cpath = home / "voice-corpus" / "corpus-work-spoken.txt"
    if not cpath.exists() or "--pooled" in sys.argv:
        cpath = home / "voice-corpus" / "corpus.txt"
    tpath = home / "voice-corpus" / "targets.json"
    rpath = home / "voice-corpus" / "rejections.json"
    if not cpath.exists():
        sys.exit(f"no corpus at {cpath}. Nothing can ground a voice without it.")
    corpus = scrub(cpath.read_text(encoding="utf-8"))
    targets = json.loads(tpath.read_text(encoding="utf-8")) if tpath.exists() else None
    if targets is None:
        sys.exit("no voice-corpus/targets.json. Run: python3 scripts/derive_voice_targets.py")
    ...
    print("# Generated by scripts/voice_brief.py from voice-corpus/. Do not paraphrase this")
    ...
    if rejections:
        ...
        print()
        print("scripts/spoken_lint.py FAILS on every one of these, so a draft carrying one")
    ...
    print("it is not done. That question is run mechanically by scripts/turing_check.py.")
